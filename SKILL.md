...
### Step 3: 체크리스트 (수동 확인)
- [ ] 모든 화살표의 머리/꼬리가 도형에 연결되어 있는가?
- [ ] 화살표 shaft가 최소 0.44" 이상인가?
- [ ] 화살표 라벨이 잘려 보이지 않는 곳은 없는가?
- [ ] 참조번호가 박스 내부 맨 첫 줄에 있는가? (뒤에 붙이지 말 것)
- [ ] 참조번호 뒤에 개행(\n)이 있는가?
- [ ] 모든 박스 텍스트가 잘림 없이 완전히 보이는가?
- [ ] dead-end 노드(출력 화살표 없는 중간 박스)가 없는가?
- [ ] 화살표가 다른 박스 텍스트를 관통(취소선처럼 보임)하지 않는가?
- [ ] 화살촉(▶)이 도착 박스의 흰색 fill에 가려지지 않는가?
