...
- ✅ 참조번호가 박스 내부 맨 첫 줄에 있는가? (검증 #6)
- ✅ dead-end 노드(출력 화살표 없는 중간 박스)가 없는가? (검증 #9)
- ✅ 화살표가 다른 박스 텍스트를 관통하지 않는가? (검증 #10)
...
| 9 | Dead-end 박스 | 입력↑ 출력=0인 중간 박스 자동 감지 |
| 10 | 화살표 관통 | 화살표 경로가 박스 텍스트 영역 관통 감지 |
